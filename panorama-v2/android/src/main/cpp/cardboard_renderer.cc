...
Matrix4x4 CardboardRenderer::GetPose() {
  ...
  CardboardHeadTracker_getPose(
      head_tracker_, GetBootTimeNano() + kPredictionTimeNanos, kLandscapeLeft,
      out_position.data(), out_orientation.data());
  return Quatf::FromXYZW(out_orientation.data()).ToMatrix();
}
...
void CardboardRenderer::OnDrawFrame() {
  ...
  for (int eye = 0; eye < 2; ++eye) {
    glViewport(eye == kLeft ? 0 : half, 0, half, screen_height_);
