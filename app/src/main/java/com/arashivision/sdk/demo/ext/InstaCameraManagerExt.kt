...
fun isPrimaryConnect(connectType: Int) =
    connectType in
        arrayOf(
...
fun setCaptureSettingValue(
    ...
    EV_INTERVAL ->
        instaCameraManager.setEVInterval(
            captureMode,
            value as EVInterval,
            checker,
        )
    ...
    SHUTTER_MODE ->
        instaCameraManager.setShutterMode(
            captureMode,
            value as ShutterMode,
            checker,
        )

    ISO -> instaCameraManager.setISO(captureMode, value as ISO, checker)
    ISO_TOP_LIMIT ->
        instaCameraManager.setISOTopLimit(
            captureMode,
            value as ISOTopLimit,
            checker,
        )

    RECORD_RESOLUTION ->
        instaCameraManager.setRecordResolution(
            captureMode,
            value as RecordResolution,
            checker,
        )

    PHOTO_RESOLUTION ->
        instaCameraManager.setPhotoResolution(
            captureMode,
            value as PhotoResolution,
            checker,
        )
    ...
    RECORD_DURATION ->
        instaCameraManager.setRecordDuration(
            captureMode,
            value as RecordDuration,
            checker,
        )

    DARK_EIS_ENABLE ->
        instaCameraManager.setDarkEisType(
            captureMode,
            value as DarkEisType,
            checker,
        )

    PANO_EXPOSURE_MODE ->
        instaCameraManager.setPanoExposureMode(
            captureMode,
            value as PanoExposureMode,
            checker,
        )

    BURST_CAPTURE ->
        instaCameraManager.setBurstCapture(
            captureMode,
            value as BurstCapture,
            checker,
        )

    INTERNAL_SPLICING ->
        instaCameraManager.setInternalSplicingEnable(
            captureMode,
            value as InternalSplicing,
            checker,
        )
    ...
    LIVE_BITRATE ->
        instaCameraManager.setLiveBitrate(
            captureMode,
            value as LiveBitrate,
            checker,
        )
...
fun getCaptureSettingSupportList(
    ...
    when (captureSetting) {
        ...
        BURST_CAPTURE ->
            instaCameraManager
                .getSupportBurstCaptureList(captureMode)
                .sortedBy { it.time }
        ...
        PHOTO_HDR_TYPE ->
            instaCameraManager
                .getSupportPhotoHdrTypeList(captureMode)
                .sortedBy { it.nativeValue }
